...
def display_instances(mysize, letter_dir, result_dir, img_file_name, image, boxes, masks, class_ids, class_names, scores=None, title="", img_h=100, img_w=100, figsize=(16, 16), ax=None, show_mask=True, show_bbox=True, colors=None, captions=None):
    """
            ...
            if show_bbox:
                ...
                if target_img_h > h or target_img_w > w:
                    ...
                    if line_count == line_list[line_idx]:
                        ...
                        io.imsave(tmp_result_path + '\\' + "%s_%s.jpg" % (img_name, str(i).zfill(3)), blank_img)
                    else:
                        fin_path = os.path.join(dst_path, img_name)
                        if not os.path.exists(fin_path):
                            os.mkdir(fin_path)
